...
variable "alerts" {
  description = <<-EOT
    Azure Monitor metric alerts for this Container App (same behavior as terraform/modules/azure/container-app-alerts).
    Set enabled = true and configure thresholds / action_group_ids; scopes use the created app's resource ID.
    When disabled, only the Container App is created (equivalent to using container-app alone).
  EOT
  type = object({
    enabled                 = optional(bool, false)
    action_group_ids        = optional(list(string), [])
    name_prefix             = optional(string, null)
    cpu_threshold           = optional(number, null)
    cpu_aggregation         = optional(string, "Maximum")
    memory_threshold        = optional(number, null)
    memory_aggregation      = optional(string, "Maximum")
    replica_count_threshold = optional(number, null)
    replica_aggregation     = optional(string, "Maximum")
    evaluation_frequency    = optional(string, "PT1M")
